...
# Renaming columns
df = df.rename(columns={'solo': 'Solo', 'open': 'Open', 'high': 'High', 'low': 'Low'}, inplace=True)
df = pd.DataFrame(df)
df.reset_index(inplace=True)
...
async def plot_chart(df):
    ...
    fig = go.Figure(data=[go.Candlestick(x=dfpl.index,
                                         ...
                                         high=dfpl['high'],
                                         low=dfpl['low']),
                          go.Scatter(x=dfpl.index, y=dfpl['EMA_slow'], line=dict(color='purple', width=1), name="EMA slow"),
                          go.Scatter(x=dfpl.index, y=dfpl['EMA_fast'], line=dict(color='orange', width=1), name="EMA fast"),
